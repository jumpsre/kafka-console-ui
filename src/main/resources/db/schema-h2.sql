...
(
    ...
    PROPERTIES   VARCHAR(4096) NOT NULL DEFAULT '' COMMENT '集群的其它属性配置',
    UPDATE_TIME  TIMESTAMP     NOT NULL DEFAULT NOW() COMMENT '更新时间',
    PRIMARY KEY (ID),
    UNIQUE (CLUSTER_NAME)
);
-- 老库升级：如果列已存在但仍是 1024，把它扩展到 4096（Kerberos 的 sasl.jaas.config 较长）
ALTER TABLE T_CLUSTER_INFO ALTER COLUMN PROPERTIES VARCHAR(4096) NOT NULL DEFAULT '';
